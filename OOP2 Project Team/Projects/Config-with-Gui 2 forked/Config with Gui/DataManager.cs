...
namespace Leave
{
	namespace DataClients
	{
		class DataManager
		{
			...
			public DataTable Trim( )	{
				DataTable data = this.data.Copy( );
				for(int i = data.Rows.Count - 1; i > 0; i--)	{
					StringBuilder currRowValue = new StringBuilder( ), prevRowValue = new StringBuilder( );
					foreach(DataColumn column in data.Columns)	{
						currRowValue.Append( data.Rows[i][column] );
						prevRowValue.Append( data.Rows[i - 1][column] );
					}
					if( currRowValue.ToString( ) == prevRowValue.ToString( ) )
